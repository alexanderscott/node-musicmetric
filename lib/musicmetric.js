...
MusicMetric.prototype.getArtistTopCities = function(opts, cb){
    ...
    self._get('/artist/'+this._idStr(opts)+'/demographics/'+opts.network+'/location/'+ opts.location, '', cb);
};
...
MusicMetric.prototype.getArtistCharts = function(opts, cb){
    ...
    var chart;
    if(!opts.chartId && _und.isString(opts.chartName)) chart = _chartUID(opts.chartName);
    else if(opts.chartId) chart = opts.chartId;
    else chart = _chartUID(this._defaults.chartName);
