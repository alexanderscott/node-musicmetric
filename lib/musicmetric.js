...
// ID HELPERS
function _getIdFromService (serviceName, serviceId, callback){
    this._get('/artist/'+serviceName+':'+serviceId, '', callback);
}

function _checkForId (opts, callback){
    if(opts.serviceName){
        this._get('/artist/'+serviceName+':'+serviceId, '', callback);
    }
...
Musicmetric.prototype.getArtistTopCities = function(opts, callback){
    _checkForId(opts, function(artistId){
        _get('/artist/'+artistId+'/demographics/'+(opts.network || 'twitter')+'/location/'+(opts.location || opts.city), '', callback);
    });
};
