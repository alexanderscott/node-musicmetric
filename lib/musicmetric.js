...
function MusicMetric(opts){
    ...
    if(_.isString(opts)) this._apiKey = opts;
...
MusicMetric.prototype._get = function(opts, cb){
...
MusicMetric.prototype.getArtistTopCities = function(opts, cb){
    ...
    this._checkForId(opts, function(err, artistId){
        self._get('/artist/'+artistId+'/demographics/'+(opts.network || 'twitter')+'/location/'+(opts.location || opts.city), '', cb);
    });
};

MusicMetric.prototype.getArtistCharts = function(opts, cb){
    this._get('/chart/'+_chartUID(opts.chart), '', cb);
};
